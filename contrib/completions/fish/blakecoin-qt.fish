...
complete --command blakecoin-qt --no-files

# Extract options
function __fish_blakecoinqt_get_options
...
complete \
    --command blakecoin-qt \
    --arguments "(__fish_blakecoinqt_get_options)"
# Enable file completions only if the commandline now contains a `*.=` style option
complete -c blakecoin-qt \
...
complete \
    --command blakecoin-qt \
    --arguments "(__fish_blakecoinqt_get_options --nofiles)"
