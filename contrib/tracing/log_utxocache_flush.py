...
"""Example logging Blakecoin Core utxo set cache flushes utilizing
    the utxocache:flush tracepoint."""

# USAGE:  ./contrib/tracing/log_utxocache_flush.py path/to/blakecoind
...
def main(blakecoind_path):
    blakecoind_with_usdts = USDT(path=str(blakecoind_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    blakecoind_with_usdts.enable_probe(
        probe="flush", fn_name="trace_flush")
    b = BPF(text=program, usdt_contexts=[blakecoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/blakecoind")
        exit(1)
