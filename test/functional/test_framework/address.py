...
ADDRESS_BCRT1_UNSPENDABLE = 'rblc1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqg4rvkm'
ADDRESS_BCRT1_UNSPENDABLE_DESCRIPTOR = 'addr(rblc1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqg4rvkm)#9x0nzqjh'
# Coins sent to this address can be spent with a witness stack of just OP_TRUE
ADDRESS_BCRT1_P2WSH_OP_TRUE = 'rblc1qft5p2uhsdcdc3l2ua4ap5qqfg4pjaqlp250x7us7a8qqhrxrxfsqf2zd4a'
...
# Blakecoin regtest/mainnet use xpub/xprv version bytes with Blake Base58Check.
...
def create_deterministic_address_bcrt1_p2tr_op_true():
    """
    Generates a deterministic Blakecoin regtest bech32m address (segwit v1 output) that
    ...
    assert_equal(address, 'rblc1p9yfmy5h72durp7zrhlw9lf7jpwjgvwdg0jr0lqmmjtgg83266lqsq9z4y5')
...
def keyhash_to_p2pkh(hash, main=False):
    assert len(hash) == 20
    version = 26 if main else 26
    return byte_to_base58(hash, version)

def scripthash_to_p2sh(hash, main=False):
    assert len(hash) == 20
    version = 7 if main else 7
    return byte_to_base58(hash, version)
...
def program_to_witness(version, program, main=False):
    ...
    return encode_segwit_address("blc" if main else "rblc", version, program)
...
def bech32_to_bytes(address):
    ...
    if hrp not in ['bc', 'tb', 'bcrt', 'blc', 'tblc', 'rblc']:
        return (None, None)
...
def address_to_scriptpubkey(address):
    ...
    if version in (26, 142, 111):  # Blakecoin main/regtest, Blakecoin testnet, legacy Bitcoin testnet
        return keyhash_to_p2pkh_script(payload)
    elif version in (7, 170, 196):  # Blakecoin main/regtest, Blakecoin testnet, legacy Bitcoin testnet
