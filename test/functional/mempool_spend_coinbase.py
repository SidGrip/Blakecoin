...
class MempoolSpendCoinbaseTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # The cached chain pays MiniWallet in blocks 76-100. Keep using the
        # same mature/immature pair as upstream, but top up the chain so the
        # pair sits exactly on the maturity boundary for Blakecoin.
        mature_coinbase_height = 99
        chain_height = mature_coinbase_height + COINBASE_MATURITY - 1
        target_height = chain_height + 2
        ...
        utxo_mature = wallet.get_utxo(txid=coinbase_txid(mature_coinbase_height))
        utxo_immature = wallet.get_utxo(txid=coinbase_txid(mature_coinbase_height + 1), include_immature_coinbase=True)
