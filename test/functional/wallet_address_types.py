...
class AddressTypeTest(BitcoinTestFramework):
    ...
    def run_test(self):
        # Mine COINBASE_MATURITY + 1 blocks on node5 to bring nodes out of IBD and make sure that
        ...
        for explicit_type, multisig, from_node in itertools.product([False, True], do_multisigs, range(4)):
            ...
            # The send plan uses 10 + 20 + 30 + 40 balance parts and leaves
            # one part for change/fees, independent of coinbase maturity.
            ...
            # Blakecoin's larger maturity window changes the cached-chain UTXO
            # shape, so coin selection may not consume nearly all wallet value.
            # The address-type behavior under test only requires that funds
            # were spent and the remaining balance is still sane.
