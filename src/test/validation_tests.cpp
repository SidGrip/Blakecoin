...
static void TestConstantSubsidy(const Consensus::Params& consensusParams)
{
    BOOST_CHECK_EQUAL(GetBlockSubsidy(0, consensusParams), 50 * COIN);
    BOOST_CHECK_EQUAL(GetBlockSubsidy(1, consensusParams), 50 * COIN);
    BOOST_CHECK_EQUAL(GetBlockSubsidy(150, consensusParams), 50 * COIN);
    BOOST_CHECK_EQUAL(GetBlockSubsidy(210000, consensusParams), 50 * COIN);
    BOOST_CHECK_EQUAL(GetBlockSubsidy(14000000, consensusParams), 50 * COIN);
}

static void TestConstantSubsidyWithInterval(int nSubsidyHalvingInterval)
{
    Consensus::Params consensusParams;
    consensusParams.nSubsidyHalvingInterval = nSubsidyHalvingInterval;
    TestConstantSubsidy(consensusParams);
}
...
BOOST_AUTO_TEST_CASE(block_subsidy_test)
{
    ...
    TestConstantSubsidy(main_params->GetConsensus()); // Blakecoin mainnet
    TestConstantSubsidy(testnet_params->GetConsensus()); // Blakecoin testnet
    TestConstantSubsidy(regtest_params->GetConsensus()); // Blakecoin regtest
    TestConstantSubsidyWithInterval(1000); // Just another interval
}
...
BOOST_AUTO_TEST_CASE(subsidy_limit_test)
{
    ...
    BOOST_CHECK_EQUAL(nSum, CAmount{70000000000000000LL});
}

BOOST_AUTO_TEST_CASE(coinbase_maturity_restore_height)
{
    BOOST_CHECK_EQUAL(COINBASE_MATURITY_LEGACY_COMPAT, 100);
    BOOST_CHECK_EQUAL(COINBASE_MATURITY, 120);
    BOOST_CHECK_EQUAL(CoinbaseMaturity(COINBASE_MATURITY_RESTORE_HEIGHT - 1), COINBASE_MATURITY_LEGACY_COMPAT);
    BOOST_CHECK_EQUAL(CoinbaseMaturity(COINBASE_MATURITY_RESTORE_HEIGHT), COINBASE_MATURITY);
    BOOST_CHECK_EQUAL(CoinbaseMaturity(COINBASE_MATURITY_RESTORE_HEIGHT + 1), COINBASE_MATURITY);
}
...
BOOST_AUTO_TEST_CASE(test_assumeutxo)
{
    ...
    BOOST_CHECK_EQUAL(
        ...
        "2b0d14d4029d47cd977ebadd28c37c9aa569218a7b7c20426f8a1e505f66a5d2");
    BOOST_CHECK_EQUAL(snapshot_130->nChainTx, 131U);

    for (const int empty : {0, 100, 110, 111, 115, 131, 200, 209, 211}) {
    ...
    const auto main_snapshot = ExpectedAssumeutxo(1987888, *main_params);
    BOOST_REQUIRE(main_snapshot);
    BOOST_CHECK_EQUAL(
        main_snapshot->hash_serialized.ToString(),
        "38c4d9036604784a891b7632c5948120760bf4750c24bf407862411845fb9f61");
    BOOST_CHECK_EQUAL(main_snapshot->nChainTx, 2878384U);
}
