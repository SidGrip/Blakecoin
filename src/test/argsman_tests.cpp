...
BOOST_AUTO_TEST_CASE(config_file_path_uses_blakecoin_name_only)
{
    ArgsManager args;
    args.ForceSetArg("-datadir", fs::PathToString(m_path_root));

    const fs::path preferred{m_path_root / "blakecoin.conf"};
    const fs::path legacy{m_path_root / "bitcoin.conf"};
    std::ofstream{legacy};

    BOOST_CHECK_EQUAL(args.GetConfigFilePath(), preferred);

    args.ForceSetArg("-conf", fs::PathToString(legacy));
    BOOST_CHECK_EQUAL(args.GetConfigFilePath(), legacy);
}
