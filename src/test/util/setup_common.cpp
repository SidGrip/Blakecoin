    ...
    : TestingSetup{CBaseChainParams::REGTEST, extra_args, coins_db_in_memory, block_tree_db_in_memory}
{
    ...
    // Generate the historical low-height regtest chain long enough for one
    // mature coinbase. Blakecoin restores 120-block maturity only at the
    // configured restore height, so low-height tests retain the 100-block
    // legacy compatibility rule.
    this->mineBlocks(COINBASE_MATURITY_LEGACY_COMPAT);

    {
        LOCK(::cs_main);
        const auto tip_hash = m_node.chainman->ActiveChain().Tip()->GetBlockHash().ToString();
        if (tip_hash != "186fcee7dc0914ec8455e164c2ebf3c60a6ecd6586eb98c55831c4f4db6343f3") {
