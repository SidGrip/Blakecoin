...
void MinerTestingSetup::TestBasicMining(const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst, int baseheight)
{
    ...
    // Blakecoin 0.25.2 enforces relative locks during block assembly here, so
    ...
    // The extra height keeps the absolute and relative lock checks aligned under Blakecoin maturity.
    for (int i = 0; i < CBlockIndex::nMedianTimeSpan; ++i) {
        CBlockIndex* ancestor{Assert(m_node.chainman->ActiveChain().Tip()->GetAncestor(m_node.chainman->ActiveChain().Tip()->nHeight - i))};
...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    for (const auto extranonce : BLOCK_EXTRANONCES) {
        ...
        CBlock *pblock = &pblocktemplate->block; // pointer for convenience
        {
            ...
            // Blakecoin regtest starts from the historical genesis nBits. Push
