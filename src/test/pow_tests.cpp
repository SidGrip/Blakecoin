...
void sanity_check_chainparams(const ArgsManager& args, std::string chainName)
{
    ...
    if (chainName == CBaseChainParams::MAIN) {
        BOOST_CHECK_EQUAL(consensus.BIP34Height, 2002950);
        BOOST_CHECK(consensus.BIP34Hash.IsNull());
        BOOST_CHECK_EQUAL(consensus.BIP65Height, 2002950);
        BOOST_CHECK_EQUAL(consensus.BIP66Height, 2002950);
        BOOST_CHECK_EQUAL(consensus.SegwitHeight, 1978980);
        const auto& taproot_deployment = consensus.vDeployments[Consensus::DEPLOYMENT_TAPROOT];
        BOOST_CHECK_EQUAL(taproot_deployment.bit, 2);
        BOOST_CHECK_EQUAL(taproot_deployment.nStartTime, int64_t{1782871200});
        BOOST_CHECK_EQUAL(taproot_deployment.nTimeout, int64_t{1814407200});
        BOOST_CHECK_EQUAL(taproot_deployment.min_activation_height, 2006310);
    }
    ...
    if (chainName == CBaseChainParams::TESTNET) {
        ...
        const auto& taproot_deployment = consensus.vDeployments[Consensus::DEPLOYMENT_TAPROOT];
        BOOST_CHECK_EQUAL(taproot_deployment.bit, 2);
        BOOST_CHECK_EQUAL(taproot_deployment.nStartTime, Consensus::BIP9Deployment::ALWAYS_ACTIVE);
        BOOST_CHECK_EQUAL(taproot_deployment.nTimeout, Consensus::BIP9Deployment::NO_TIMEOUT);
        BOOST_CHECK_EQUAL(taproot_deployment.min_activation_height, 0);
    }
}
