#!/bin/bash
# =============================================================================
# Blakecoin 0.25.2 Build Script  -  All Platforms
#
...
#
# Repository: https://github.com/SidGrip/Blakecoin.git (branch: 0.25.2)
# =============================================================================
...
QT_WM_CLASS="Blakecoin-Qt"
QT_DESKTOP_ID="org.blakecoin.blakecoin-qt"
...
WINDOWS_ICON_SOURCE_PNG="$SCRIPT_DIR/src/qt/res/icons/blakecoin.png"
WINDOWS_ICON_SOURCE_TESTNET_PNG="$SCRIPT_DIR/src/qt/res/icons/blakecoin_testnet.png"
...
sync_windows_icon_assets() {
    ...
    info "Regenerating Windows icon assets from repo blakecoin.png sources..."
    python3 - <<PY
...
\`\`\`bash
cp blakecoind blakecoin-cli blakecoin-tx blakecoin-wallet blakecoin-util ~/.local/bin/
\`\`\`
...
install_linux_desktop_launcher() {
    ...
    local icon_source="$SCRIPT_DIR/src/qt/res/icons/blakecoin.png"
...
DEOF
    ...
    info "Desktop launcher installed  -  Blakecoin will appear in Activities search"
}
...
${HOST}-strip src/blakecoin-wallet.exe 2>/dev/null || true
${HOST}-strip src/blakecoin-util.exe 2>/dev/null || true

echo ">>> Build complete!"
ls -lh src/blakecoind.exe src/qt/blakecoin-qt.exe src/blakecoin-cli.exe src/blakecoin-tx.exe src/blakecoin-wallet.exe src/blakecoin-util.exe 2>/dev/null || true
'
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        ...
        docker cp "$container_name:/build/blakecoin/src/blakecoind.exe" "$output_dir/blakecoind-${VERSION}.exe" 2>/dev/null || true
        docker cp "$container_name:/build/blakecoin/src/blakecoin-cli.exe" "$output_dir/blakecoin-cli-${VERSION}.exe" 2>/dev/null || true
        docker cp "$container_name:/build/blakecoin/src/blakecoin-tx.exe" "$output_dir/blakecoin-tx-${VERSION}.exe" 2>/dev/null || true
        docker cp "$container_name:/build/blakecoin/src/blakecoin-wallet.exe" "$output_dir/blakecoin-wallet-${VERSION}.exe" 2>/dev/null || true
        docker cp "$container_name:/build/blakecoin/src/blakecoin-util.exe" "$output_dir/blakecoin-util-${VERSION}.exe" 2>/dev/null || true
    fi

    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        info "Extracting Qt wallet..."
        docker cp "$container_name:/build/blakecoin/src/qt/blakecoin-qt.exe" "$output_dir/blakecoin-qt-${VERSION}.exe" 2>/dev/null || true
    fi
...
# Generate .icns icon from blakecoin.png
ICONS_DIR="src/qt/res/icons"
if [ -f "$ICONS_DIR/blakecoin.png" ]; then
    echo ">>> Generating macOS icon from blakecoin.png..."
    apt-get update -qq >/dev/null 2>&1 || true
    apt-get install -y -qq python3-pil >/dev/null 2>&1 || true
    python3 -c "
from PIL import Image
img = Image.open('"'"'$ICONS_DIR/blakecoin.png'"'"')
...
'
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        info "Extracting daemon binaries..."
        docker cp "$container_name:/build/blakecoin/src/blakecoind" "$output_dir/blakecoind-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/blakecoin/src/blakecoin-cli" "$output_dir/blakecoin-cli-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/blakecoin/src/blakecoin-tx" "$output_dir/blakecoin-tx-${VERSION}" 2>/dev/null || true
    fi
    ...
    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        ...
        docker cp "$container_name:/build/blakecoin/src/qt/blakecoin-qt" "$output_dir/blakecoin-qt-${VERSION}" 2>/dev/null || true
    fi
...
' >/dev/null
    ...
    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        ...
        if docker cp "$container_name:/build/blakecoin/Blakecoin-Qt.app" "$output_dir/Blakecoin-Qt.app" 2>/dev/null; then
...
mkdir -p "$ICON_DIR"
if [ -f src/qt/res/icons/blakecoin.png ]; then
    cp src/qt/res/icons/blakecoin.png "$ICON_DIR/'"$COIN_NAME"'.png"
else
...
strip src/blakecoin-wallet 2>/dev/null || true
strip src/blakecoin-util 2>/dev/null || true

echo ">>> Build complete!"
ls -lh src/blakecoind src/qt/blakecoin-qt src/blakecoin-cli src/blakecoin-tx src/blakecoin-wallet src/blakecoin-util 2>/dev/null || true
'
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        info "Extracting daemon binaries..."
        docker cp "$container_name:/build/blakecoin/src/blakecoind" "$daemon_stage/blakecoind-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/blakecoin/src/blakecoin-cli" "$daemon_stage/blakecoin-cli-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/blakecoin/src/blakecoin-tx" "$daemon_stage/blakecoin-tx-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/blakecoin/src/blakecoin-wallet" "$daemon_stage/blakecoin-wallet-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/blakecoin/src/blakecoin-util" "$daemon_stage/blakecoin-util-${VERSION}" 2>/dev/null || true
    fi

    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        info "Extracting Qt wallet..."
        docker cp "$container_name:/build/blakecoin/src/qt/blakecoin-qt" "$qt_stage/blakecoin-qt-${VERSION}" 2>/dev/null || true
    fi
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        cp "$daemon_stage/blakecoin-wallet-${VERSION}" "$final_output_dir/$WALLET_NAME" 2>/dev/null || warn "Missing Linux wallet tool artifact: $daemon_stage/blakecoin-wallet-${VERSION}"
        cp "$daemon_stage/blakecoin-util-${VERSION}" "$final_output_dir/$UTIL_NAME" 2>/dev/null || warn "Missing Linux util tool artifact: $daemon_stage/blakecoin-util-${VERSION}"
    fi
...
QRC_EOF
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        ...
        relink_native_linux_target "blakecoin-wallet" "$SCRIPT_DIR/src/blakecoin-wallet"
        relink_native_linux_target "blakecoin-util" "$SCRIPT_DIR/src/blakecoin-util"
    fi
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        strip src/blakecoind src/blakecoin-cli src/blakecoin-tx src/blakecoin-wallet src/blakecoin-util 2>/dev/null || true
    fi
    ...
    finalize_linux_native_output \
	    ...
	    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        cp "$SCRIPT_DIR/src/blakecoin-wallet" "$final_output_dir/$WALLET_NAME" 2>/dev/null || warn "Missing Linux wallet tool artifact: $SCRIPT_DIR/src/blakecoin-wallet"
        cp "$SCRIPT_DIR/src/blakecoin-util" "$final_output_dir/$UTIL_NAME" 2>/dev/null || warn "Missing Linux util tool artifact: $SCRIPT_DIR/src/blakecoin-util"
    fi
...
QRC_EOF
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        ...
        cp src/blakecoin-wallet "$output_dir/blakecoin-wallet-${VERSION}"
        cp src/blakecoin-util "$output_dir/blakecoin-util-${VERSION}"
    ...
    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        ...
        if [[ -f "$icons_dir/blakecoin.png" ]] && command -v sips &>/dev/null && command -v iconutil &>/dev/null; then
            info "Generating macOS icon from blakecoin.png..."
            local iconset_root iconset_dir size size2
            iconset_root=$(mktemp -d)
            iconset_dir="$iconset_root/blakecoin.iconset"
            mkdir -p "$iconset_dir"
            for size in 16 32 128 256 512; do
                sips -z "$size" "$size" "$icons_dir/blakecoin.png" --out "$iconset_dir/icon_${size}x${size}.png" >/dev/null 2>&1 || true
                size2=$((size * 2))
                sips -z "$size2" "$size2" "$icons_dir/blakecoin.png" --out "$iconset_dir/icon_${size}x${size}@2x.png" >/dev/null 2>&1 || true
            done
...
PY
    ...
    # Blakecoin 0.25.2 does not ship the upstream translation payloads.
    if [[ -f src/Makefile ]]; then
        sedi 's/^QT_QM.*=.*/QT_QM =/' src/Makefile
        sedi '/bitcoin_.*\.qm/d' src/Makefile
        sedi '/locale\/.*\.qm/d' src/Makefile
